...
// ── フェーズ管理フラグ ────────────────────────────────────
// フェーズ1（信頼構築優先）では課金機能をすべて無料開放する。
// 成約実績が積み上がり、宅建業法該当性の確認が取れた段階で true に切り替える。
...
function VerifiedBadge({ size="normal" }) {
  ...
  return (
    <span style={{
      ...
      borderRadius:20, padding: small?"1px 7px":"3px 10px", fontSize: small?9:11,
      color:"#1D4ED8", fontWeight:700, display:"inline-flex", alignItems:"center", gap:3
    }}>✅ 本人確認済み</span>
  );
}

// 無料期間中であることを示す共通バッジ
function FreeBadge({ children="無料公開中" }) {
  return (
    <span style={{
...
// 登録内容の充実度から簡易な信頼度スコアを算出（0-100）
function calcTrustScore(farm) {
  const checks = [
    farm.owner_verified,
    farm.owner_bio,
...
function OwnerTrustPanel({ farm }) {
  const trustScore = calcTrustScore(farm);
  const hasAnyInfo = farm.owner_bio || farm.reason_for_listing || farm.response_time_estimate || farm.past_crop_history || farm.owner_verified;
  if (!hasAnyInfo) return null;
  return (
    <div style={{ marginBottom:14, background:"#F8FBF3", border:`1px solid #DCEBC4`, borderRadius:10, padding:"14px 16px" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
        <div style={{ fontSize:12, fontWeight:700, color:C.green, display:"flex", alignItems:"center", gap:6 }}>
          🤝 オーナー情報（安心してご相談いただくために）
        </div>
        {farm.owner_verified && <VerifiedBadge size="small"/>}
      </div>

      {trustScore > 0 && (
        <div style={{ marginBottom:10 }}>
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:C.muted, marginBottom:4 }}>
            <span>登録情報の充実度</span><span>{trustScore}%</span>
          </div>
          <div style={{ height:6, background:C.border, borderRadius:3, overflow:"hidden" }}>
            <div style={{ height:"100%", width:`${trustScore}%`, background:C.lightGreen, borderRadius:3 }}/>
          </div>
        </div>
      )}
      ...
      {farm.reason_for_listing && (
        <div style={{ marginBottom:8 }}>
          <div style={{ fontSize:10, color:C.muted, marginBottom:2 }}>この農地を貸したい理由</div>
          <div style={{ fontSize:12, color:C.text, lineHeight:1.6 }}>{farm.reason_for_listing}</div>
        </div>
      )}
      {farm.owner_bio && (
        <div style={{ marginBottom:8 }}>
          <div style={{ fontSize:10, color:C.muted, marginBottom:2 }}>オーナーより一言</div>
          <div style={{ fontSize:12, color:C.text, lineHeight:1.6 }}>{farm.owner_bio}</div>
        </div>
      )}
      {farm.past_crop_history && (
        <div style={{ marginBottom:8 }}>
          <div style={{ fontSize:10, color:C.muted, marginBottom:2 }}>これまでの栽培実績</div>
          <div style={{ fontSize:12, color:C.text, lineHeight:1.6 }}>{farm.past_crop_history}</div>
        </div>
      )}
      {farm.response_time_estimate && (
        <div style={{ fontSize:11, color:C.green, fontWeight:600 }}>
          📩 返信目安：{farm.response_time_estimate}
        </div>
      )}
    </div>
  );
  ...
  const plans = [
    { id:"owner", who:"農地・物件オーナー", emoji:"🏡", color:C.soil, bg:C.soilLight, border:C.soilBorder,
      items:[
        ...
        {name:"成約報酬",price:"1件あたり定額 1〜3万円（予定）",desc:"マッチング成立時のみ発生。成約しなければ追加費用なし。"},
      ]},
    { id:"seeker", who:"就農希望者・移住希望者", emoji:"🌱", color:C.green, bg:C.paleGreen, border:"#B8D98A",
      items:[
        {name:"無料プラン",price:"¥0",desc:"農地・住居の一覧閲覧。"},
        {name:"プレミアム会員",price:"¥500〜1,500/月（予定）",desc:"早期閲覧・オーナーへの直接連絡権など。"},
        {name:"体験ツアー予約",price:"予約額の一部（予定）",desc:"農業体験・見学ツアーの仲介手数料。参加費は別途。"},
      ]},
  ];
  return (
    <div>
      ...
      <div style={{ background:"#F0FDF4", border:"2px solid #86EFAC", borderRadius:12,
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
          ...
          <span style={{ fontWeight:800, fontSize:15, color:"#166534" }}>現在、全機能を無料でご利用いただけます</span>
        </div>
        <p style={{ fontSize:12, color:"#166534", margin:0, lineHeight:1.7 }}>
          Farmatchは今、農地の登録数と信頼できる情報の厚みを増やすことを最優先にしています。
          そのため掲載料・成功報酬・プレミアム会員費はすべて<b>当面無料</b>です。
          成約実績が積み上がり、必要な法令確認が整い次第、段階的に下記の料金プランへ移行する予定です。
          料金が発生する際は事前に告知します。
        </p>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        {plans.map(p=>(
          <div key={p.id} style={{ background:p.bg, border:`2px solid ${p.border}`, borderRadius:12, padding:20, position:"relative" }}>
            <div style={{ position:"absolute", top:14, right:14 }}><FreeBadge/></div>
            <div style={{ fontSize:24, marginBottom:6 }}>{p.emoji}</div>
            <div style={{ fontWeight:700, color:p.color, fontSize:15, marginBottom:4 }}>{p.who}</div>
            <div style={{ fontSize:11, color:C.muted, marginBottom:12 }}>導入予定の料金プラン（現在は全項目無料）</div>
            {p.items.map(pl=>(
              <div key={pl.name} style={{ background:C.white, borderRadius:8, padding:"12px 14px",
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:4 }}>
                  ...
                  <span style={{ fontWeight:800, color:C.muted, fontSize:11, whiteSpace:"nowrap", marginLeft:8, textDecoration:"line-through" }}>{pl.price}</span>
                </div>
...
function AdminPanel({ farms, houses, onRefresh, onLogout }) {
  ...
  const [form, setForm] = useState({
    ...
    // 信頼構築項目（フェーズ1で優先的に充実させる任意項目）
    owner_verified:false, owner_bio:"", reason_for_listing:"",
    response_time_estimate:"", past_crop_history:"",
  });
  ...
  const openEdit = (item, type) => {
    ...
    setForm({
      ...
      response_time_estimate:item.response_time_estimate||"",
  ...
  const handleSave = async()=>{
    ...
    const payload = formType==="farm"
      ? { name:form.name, region:form.region, location:form.location,
          ...
          reason_for_listing:form.reason_for_listing,
  ...
  const FARM_COLS = [
    ...
    {key:"owner_verified",label:"本人確認済み"},{key:"owner_bio",label:"オーナー自己紹介"},
    {key:"reason_for_listing",label:"貸したい理由"},{key:"response_time_estimate",label:"返信目安"},
    {key:"past_crop_history",label:"過去の栽培実績"},
        ...
        <Modal onClose={()=>setShowForm(false)}>
          ...
          {formType==="farm" && (
            <div style={{ background:"#F8FBF3", border:"1px solid #DCEBC4", borderRadius:8, padding:"12px 14px", marginBottom:12 }}>
              <div style={{ fontSize:11, color:C.green, fontWeight:700, marginBottom:8, display:"flex", alignItems:"center", gap:6 }}>
                🤝 信頼構築情報（任意・入力するほど掲載の安心感が高まります）
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
                ...
                <label htmlFor="owner_verified" style={{ fontSize:12, color:C.text, cursor:"pointer" }}>
                  ✅ 運営側で本人確認を完了している
                </label>
              </div>
              <div style={{ marginBottom:8 }}>
                <label style={{ fontSize:11, color:C.green, fontWeight:600, display:"block", marginBottom:3 }}>この農地を貸したい理由</label>
                <input value={form.reason_for_listing} onChange={e=>setForm({...form,reason_for_listing:e.target.value})}
                  placeholder="例：後継者がおらず、有効活用してくれる方に貸したい"
                  style={{ width:"100%", border:`1.5px solid ${C.border}`, borderRadius:8,
                    padding:"8px 10px", fontSize:13, boxSizing:"border-box", outline:"none" }}/>
              </div>
              <div style={{ marginBottom:8 }}>
                <label style={{ fontSize:11, color:C.green, fontWeight:600, display:"block", marginBottom:3 }}>オーナーの自己紹介</label>
                <textarea value={form.owner_bio} onChange={e=>setForm({...form,owner_bio:e.target.value})} rows={2}
                  placeholder="例：地元で長年農業を営んできました。丁寧に土づくりのコツをお伝えします。"
                  style={{ width:"100%", border:`1.5px solid ${C.border}`, borderRadius:8,
                    padding:"8px 10px", fontSize:13, boxSizing:"border-box", resize:"vertical", outline:"none" }}/>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                <div>
                  <label style={{ fontSize:11, color:C.green, fontWeight:600, display:"block", marginBottom:3 }}>返信目安</label>
                  <input value={form.response_time_estimate} onChange={e=>setForm({...form,response_time_estimate:e.target.value})}
                    placeholder="例：2〜3日以内"
                    style={{ width:"100%", border:`1.5px solid ${C.border}`, borderRadius:8,
                      padding:"8px 10px", fontSize:13, boxSizing:"border-box", outline:"none" }}/>
                </div>
                <div>
                  <label style={{ fontSize:11, color:C.green, fontWeight:600, display:"block", marginBottom:3 }}>過去の栽培実績</label>
                  <input value={form.past_crop_history} onChange={e=>setForm({...form,past_crop_history:e.target.value})}
                    placeholder="例：これまでさつまいもを栽培"
                    style={{ width:"100%", border:`1.5px solid ${C.border}`, borderRadius:8,
                      padding:"8px 10px", fontSize:13, boxSizing:"border-box", outline:"none" }}/>
                </div>
              </div>
    ...
    <div style={{ background:C.white, borderRadius:12, border:`2px solid ${C.border}`, padding:22, position:"sticky", top:16 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:C.muted, marginBottom: farm.owner_verified?6:0 }}>📍 {farm.region}　{farm.location}</div>
          {farm.owner_verified && <VerifiedBadge size="small"/>}
        </div>
      ...
      {farm.is_premium && !isPremium && PAID_FEATURES_ACTIVE ? (
        ...
        <>
          {farm.is_premium && (
            <div style={{ marginBottom:10, display:"flex", justifyContent:"flex-end" }}>
              <FreeBadge>プレミアム情報も現在無料公開中</FreeBadge>
            </div>
          )}
    ...
    <div style={{ background:C.cream, minHeight:"100vh", fontFamily:"'Hiragino Kaku Gothic ProN','Noto Sans JP',sans-serif" }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {user ? (
              <>
                ...
                {isPremium ? (
                  <span style={{ background:C.soil, color:"#fff", borderRadius:20, padding:"5px 14px", fontSize:11, fontWeight:700 }}>
                    ⭐ プレミアム{!PAID_FEATURES_ACTIVE && "（無料期間中）"}
                  </span>
                ) : (
                  <button onClick={()=>setIsPremium(true)} style={{ background:C.soil, color:"#fff", border:"none", borderRadius:20, padding:"6px 14px", fontSize:11, fontWeight:700, cursor:"pointer" }}>
                    {PAID_FEATURES_ACTIVE ? "プレミアム登録" : "プレミアム機能を無料で試す"}
                  </button>
                )}
      ...
      {tab==="farms" && (
        <div style={{ background:`linear-gradient(135deg,${C.green},${C.deepGreen})`, padding:"28px 20px", textAlign:"center" }}>
          ...
          <p style={{ color:"#B8D98A", fontSize:13, margin:"0 0 14px" }}>遊休農地を活かし、新しい農業の担い手へつなぐ</p>
          <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:"rgba(255,255,255,0.12)",
            border:"1px solid rgba(255,255,255,0.25)", borderRadius:20, padding:"5px 14px", marginBottom:16 }}>
            <span style={{ fontSize:12, color:"#fff", fontWeight:700 }}>🎁 農地・物件の掲載は現在すべて無料</span>
          </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:selected?"1fr 1fr":"1fr", gap:20 }}>
              <div>
                ...
                <div style={{ background:`linear-gradient(135deg,${C.soil},#8B5A1A)`, borderRadius:12, padding:"18px 20px", color:"#fff", marginTop:8 }}>
                  ...
                  <p style={{ fontSize:12, margin:"0 0 8px", opacity:0.9, lineHeight:1.6 }}>
                    使われていない農地や空き物件を登録して、新しい農業の担い手とつながりましょう。
                    掲載料は当面無料。本人確認バッジやプロフィール、現地写真を充実させるほど、
                    借り手に安心して問い合わせてもらいやすくなります。
                  </p>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:12 }}>
                    <span style={{ background:"rgba(255,255,255,0.18)", borderRadius:20, padding:"3px 10px", fontSize:11 }}>掲載無料</span>
                    <span style={{ background:"rgba(255,255,255,0.18)", borderRadius:20, padding:"3px 10px", fontSize:11 }}>本人確認バッジ対応</span>
                    <span style={{ background:"rgba(255,255,255,0.18)", borderRadius:20, padding:"3px 10px", fontSize:11 }}>2〜3分で登録完了</span>
                  </div>
