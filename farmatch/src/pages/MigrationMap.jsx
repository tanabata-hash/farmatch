...
import { JAPAN_MAP_VIEWBOX, JAPAN_MAP_W, JAPAN_MAP_H, JAPAN_ISLANDS_INNER, JAPAN_BORDERS_INNER } from "./japanMapData";
...
export const PREF_DATA = {
  "北海道": { url:"https://www.pref.hokkaido.lg.jp/ns/gjf/a0004/b0001/",
  ...
  "栃木県": { url:"https://www.pref.tochigi.lg.jp/g04/keigi_sinkisyuunou.html",
  ...
  "奈良県": { url:"https://www.pref.nara.lg.jp/n120/39870.html",
  ...
  "和歌山県": { url:"https://www.pref.wakayama.lg.jp/prefg/070100/shiensaku/d00155006.html",
  ...
  "愛媛県": { url:"https://www.pref.ehime.jp/soshiki/80/",
  ...
  "長崎県": { url:"https://n-agrisupport.pref.nagasaki.jp/",
  ...
  "鹿児島県": { url:"http://www.pref.kagoshima.jp/sangyo-rodo/nogyo/nogyo/shinki/index.html",
...
// 実際の日本列島の輪郭データ（CC0/パブリックドメイン、出典は japanMapData.js 内に記載）
const MAP_VIEWBOX = JAPAN_MAP_VIEWBOX;

// 各地方ブロックのピン位置（実データの都道府県中心座標から代表点を採用。実際の位置関係そのまま）
const MAP_PINS = {
  "北海道": { x:1573.5, y:427.5 },   // 北海道
  "東北":   { x:1470.5, y:1237.5 },  // 宮城県
  "関東":   { x:1330.5, y:1660.5 },  // 埼玉県
  "中部":   { x:1160.5, y:1667 },    // 長野県
  "近畿":   { x:796,    y:1683.5 },  // 京都府
  "中国":   { x:614.5,  y:1919 },    // 岡山県
  "四国":   { x:709.5,  y:1976 },    // 香川県
  "九州":   { x:242,    y:2268 },    // 熊本県
  "沖縄":   { x:1732,   y:2323.5 },  // 沖縄県（実データ内でも本島から離れたインセット表示）
};
...
function Pin({ name, x, y, isSelected, onClick, small, colorOverride }) {
  const left = `${(x/JAPAN_MAP_W)*100}%`, top = `${(y/JAPAN_MAP_H)*100}%`;
...
function JapanMap({ selectedBlock, onSelectBlock }) {
  return (
    <div style={{ position:"relative", width:"100%", maxWidth:380, aspectRatio:`${JAPAN_MAP_W}/${JAPAN_MAP_H}`, margin:"0 auto 16px" }}>
      <svg viewBox={MAP_VIEWBOX} style={{ width:"100%", height:"100%", display:"block" }}>
        <rect x="0" y="0" width={JAPAN_MAP_W} height={JAPAN_MAP_H} rx="80" fill="#EAF3FC"/>
        <g fill="#DCE9C8" stroke="#9FBB7E" strokeWidth="7" dangerouslySetInnerHTML={{ __html: JAPAN_ISLANDS_INNER }}/>
        <g fill="none" stroke="#B9C99A" strokeWidth="4" dangerouslySetInnerHTML={{ __html: JAPAN_BORDERS_INNER }}/>
        {/* 沖縄は実データ内でも本州から離れた位置にインセット表示されているため、枠で囲んで示す */}
        <rect x="1790" y="2170" width="270" height="220" rx="20" fill="none" stroke="#9FBB7E" strokeWidth="3" strokeDasharray="14,10"/>
        <text x="1800" y="2150" fontSize="34" fill={C.muted} fontWeight="700">沖縄（縮尺・位置は模式的）</text>
      </svg>
