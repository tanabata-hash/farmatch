  ...
  return (
    <LegalPage title="利用規約" onBack={onBack}>
      ...
      <Section title="第9条（免責事項）">
        当サービスは、農地・物件の情報を掲載し、当事者同士が連絡を取り合うための場（プラットフォーム）を提供するものです。当サービスは契約の媒介・代理・条件交渉のあっせんを行うものではなく、実際の契約締結や交渉はユーザー間で行っていただきます。当サービスは、掲載情報の内容および当事者間の取引の結果について一切の責任を負いません。ただし、消費者契約法に定める消費者契約となる場合、この免責規定は適用されません。
      </Section>
  ...
  return (
    <LegalPage title="プライバシーポリシー" onBack={onBack}>
      ...
      <Section title="第4条（第三者サービスの利用）">
        ...
        これらのサービスへのデータ移転は、各社のデータ保護方針に基づいて行われます。日本の個人情報保護法上、米国は十分性認定国ではないため、当サービスは各社が締結する標準契約条項（SCCs）等の適切な安全管理措置に基づき、必要な範囲でデータを移転します。
      </Section>
